...
def add_results(final_results, name, result_dict, result_list, took, show_accuracy=False):
    ...
    if show_accuracy:
        ...
        if "mAP" in result_dict:
            ...
            acc_str += ", mAP={:.15f}%".format(result["mAP"])
...
def main():
    ...
    try:
        ...
        packed_backend_params = args.backend_params

        unpacked_backend_params = dict(pair.split(":") for pair in packed_backend_params.split(",")) if packed_backend_params else {}
        backend.set_extra_params( unpacked_backend_params )
        print ("[DVDT] Setting extra parametrs for {} backend: {}".format(backend.name(), unpacked_backend_params))
